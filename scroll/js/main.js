$(document).ready(function() {
	...
	var t1, t2;
	var questionNum = 5;
	var times = ['2014/09/26 14:40:00', '2014/09/15 15:32:30', '2014/09/15 11:34:00', '2014/09/15 11:36:30', '2014/09/15 11:38:00', '2014/09/09 11:45:30'];
	var timeStamps = [new Date(times[0]).getTime(), new Date(times[1]).getTime(), new Date(times[2]).getTime(), new Date(times[3]).getTime(), new Date(times[4]).getTime(), new Date(times[5]).getTime()];
	...
	function timeJudge() {
		...
		if (nowTime < timeStamps[0]) {
			questionNum = 6;
			$('.single_question').eq(5).show().removeClass('preparing');
			timer1(parseInt((timeStamps[0] - nowTime) / 1000) + 1);
		} else if (nowTime >= timeStamps[0] && nowTime < timeStamps[1]) {
			questionNum = 5;
			$('.single_question').eq(5).show().removeClass('preparing').find('.counting_circle').hide();
			timer2(parseInt((timeStamps[1] - nowTime) / 1000) + 1);
		} else if (nowTime >= timeStamps[1] && nowTime < timeStamps[2]) {
			questionNum = 4;
			$('.single_question').eq(5).show().removeClass('preparing').find('.counting_circle').hide();
			$('.single_question').eq(4).slideDown().removeClass('preparing').find('.counting_circle').hide();
			...
			checkAnswerPassed(4);
			checkAnswerPassed(5);
		} else if (nowTime >= timeStamps[4] && nowTime < timeStamps[5]) {
			//fifth question
			questionNum = 1;
			$('.single_question').eq(5).show().removeClass('preparing').find('.counting_circle').hide();
			$('.single_question').eq(4).show().removeClass('preparing').find('.counting_circle').hide();
			...
			timer6(parseInt((timeStamps[5] - nowTime) / 1000) + 1);
			checkAnswerPassed(2);
			checkAnswerPassed(3);
			checkAnswerPassed(4);
			checkAnswerPassed(5);
